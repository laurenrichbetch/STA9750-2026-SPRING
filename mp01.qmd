...
acquire_ipeds_data <- function(start_year=2010, end_year=2024){
  ...
  DESC_ALL <- map(YEARS, function(yy){
    ...
    read_csv(
      ...
      mutate(year = yy) |>   # FIXED — no stray INSTNM
      select(
        institution_id = UNITID,
...
# Identify CUNY and California Publics
IPEDS <- IPEDS |>
  mutate(
...
Loyola_yearly <- IPEDS |>
  ...
  mutate(`Grand Total` = aian + asian + black + hispanic + nhpi +
...
IPEDS_yearly_pct <- add_pct(IPEDS_yearly)
Loyola_yearly_pct <- add_pct(Loyola_yearly)
...
# ## National Enrollment YoY Trends
```{r}
IPEDS_yearly_pct |>
  gt() |>
...
# ## Loyola Maryland Enrollment YoY Trends
```{r}
Loyola_yearly_pct |>
  gt() |>
  format_gt_table("Loyola Maryland enrollment YoY trends")
```
